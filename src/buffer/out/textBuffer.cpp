...
try
{
    ...
    til::CoordType oldWidth = oldBuffer.GetSize().Width();
    ...
    for (;;)
    {
        ...
        if (oldX < oldRowLimit)
        {
            ...
            const auto copyableAmount = std::min(oldWidth - oldRowBegin, newWidth - newRowBegin);
            const auto oldAttr = oldRow.Attributes().slice(gsl::narrow_cast<uint16_t>(oldRowBegin), gsl::narrow_cast<uint16_t>(oldRowBegin + copyableAmount));
            newRow.Attributes().replace(gsl::narrow_cast<uint16_t>(newRowBegin), gsl::narrow_cast<uint16_t>(newRowBegin + copyableAmount), oldAttr);
            newRow.Attributes().resize_trailing_extent(gsl::narrow_cast<uint16_t>(newWidth));
        }
